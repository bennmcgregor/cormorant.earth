...
PUBLISH_FALSE_RE = PUBLISH_FALSE_RE = re.compile(r"""^\s*publish\s*:\s*["']?false["']?\s*$""", re.MULTILINE | re.IGNORECASE)
...
def has_publish_false(md_path: Path) -> bool:
    try:
        text = md_path.read_text(encoding="utf-8")
    except (UnicodeDecodeError, OSError):
        return False
    m = FRONTMATTER_RE.match(text)
    return bool(m and PUBLISH_FALSE_RE.search(m.group(1)))
...
def main() -> int:
    ...
    for md in VAULT_PUBLISH_DIR.rglob("*.md"):
        if is_hidden(md.relative_to(VAULT_PUBLISH_DIR)):
            continue
        all_md.add(md)
        if has_publish_false(md):
            excluded.add(md)
    published_md = all_md - excluded

    # Always show what's being held back, even when the list is empty
    if excluded:
        print(f"\n!!! {len(excluded)} note(s) marked publish: false (will NOT be published):")
        for md in sorted(excluded):
            print(f"  - {md.relative_to(VAULT_PUBLISH_DIR)}")
        print()
    else:
        print("No notes marked publish: false in publish folder.")
